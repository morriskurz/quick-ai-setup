  ...
  return (
    <Section id="your-setup" title={copy.title} intro={<p className="m-0">{copy.intro}</p>}>
      <div id="prompt" className="ccc-hairline-card scroll-mt-6 flex flex-col gap-4 p-5 sm:p-6">
        <h3 className="m-0 text-body font-semibold text-ink">{ui.setup.pathOne}</h3>
        ...
        <div className="flex flex-wrap items-center gap-x-6 gap-y-3">
          ...
          <MetaRow items={[ui.steps(plan.steps.length), OS_LABEL[selection.os]]} />
        </div>
      </div>
      ...
      <div className="mt-14">
        ...
        <div className="mt-5">
          <OsTabs value={selection.os} onChange={onSetOs} idBase="manual" panelId={panelId} label={ui.setup.osTabs} />
        </div>
