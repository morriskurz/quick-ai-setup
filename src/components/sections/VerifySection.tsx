...
export function VerifySection({ selection, script, onSetOs }: VerifySectionProps) {
  ...
  return (
    <Section
      ...
      eyebrow="Verify"
      // COPY: owner review
      lead="Check what is installed."
      gradient="In one go."
      intro={
        <p className="m-0">
          Paste this into {OS_SHELL[os]} when the setup is done. It prints the version of every tool in your setup.
        </p>
      }
    >
      ...
      <div id={panelId} role="tabpanel" aria-labelledby={tabId('verify', os)} className="mt-6">
        <div key={os} className="ccc-fade">
          <CodeBlock code={script} label={`${OS_LABEL[os]} · ${OS_SHELL[os]}`} context="verify script" />
        </div>
      </div>
    </Section>
  );
